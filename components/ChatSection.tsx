...
export function detectDeviceAuthor(): DeviceDetectionResult {
  ...
  // Screen dimensions (CSS pixels and DPR)
...
// Gentle pleasant soft chime on incoming message
...
export default function ChatSection({ onKeyboardChange, onBack }: ChatSectionProps) {
  ...
  // Scroll to bottom button visibility & new unread messages below
  const [showScrollBottom, setShowScrollBottom] = useState(false);
  const [hasUnreadBelow, setHasUnreadBelow] = useState(false);

  // Safari iOS keyboard status
  ...
  // Direct container scrolling - completely bypasses window.scroll / scrollIntoView
  // which prevents iOS Safari jitter and shaking
  ...
  // Scroll to a specific message inside the container smoothly
  const scrollToMessageElement = (elementId: string) => {
    const container = messagesContainerRef.current;
    const target = document.getElementById(elementId);
    if (!container || !target) return;
    const containerRect = container.getBoundingClientRect();
    const elRect = target.getBoundingClientRect();
    const targetScrollTop = container.scrollTop + (elRect.top - containerRect.top) - (container.clientHeight / 2) + (target.clientHeight / 2);
  ...
  const fetchMessages = useCallback(async (silent: boolean = false) => {
    ...
    try {
      ...
      if (data.success && Array.isArray(data.messages)) {
        setMessages((prev) => {
          // Fast shallow check to avoid wasteful re-renders if messages are unchanged
          const isSame =
            prev.length === data.messages.length &&
            prev.every((m, idx) => {
              const nm = data.messages[idx];
              return (
                m.id === nm.id &&
                m.text === nm.text &&
                JSON.stringify(m.reactions || {}) === JSON.stringify(nm.reactions || {})
              );
            });
          ...
          if (prev.length > 0 && data.messages.length > prev.length) {
            const newItems = data.messages.slice(prev.length);
            const partnerMsg = newItems.find((m: ChatMessage) => !isMyMessage(m));
        ...
        if (data.lastSeen) {
          setLastSeenTimes((prev) => {
            if (
              prev.Fatih === data.lastSeen.Fatih &&
              prev.Ceyda === data.lastSeen.Ceyda
            ) {
              return prev;
            }
  ...
  }, [authorName, isMyMessage, scrollToBottom]);

  // Initial load and fast 2.2s polling when tab is active
  useEffect(() => {
    fetchMessages();
  ...
  useEffect(() => {
    ...
    if (matchedMessageIds.length > 0) {
      scrollToMessageElement(`msg-${matchedMessageIds[0]}`);
    }
  }, [searchQuery, matchedMessageIds]);

  const handleNextMatch = () => {
    if (matchedMessageIds.length === 0) return;
    const nextIdx = (currentMatchIndex + 1) % matchedMessageIds.length;
    setCurrentMatchIndex(nextIdx);
    scrollToMessageElement(`msg-${matchedMessageIds[nextIdx]}`);
  };
  ...
  const handlePrevMatch = () => {
    ...
    scrollToMessageElement(`msg-${matchedMessageIds[prevIdx]}`);
  };
  ...
  const handleSendMessage = async (e?: React.FormEvent, customText?: string) => {
    ...
    // Scroll down immediately without window bouncing
    setTimeout(() => {
      scrollToBottom(true);
    }, 40);
    ...
    >
      <div className="w-full max-w-4xl mx-auto flex flex-col h-full overflow-hidden px-2 sm:px-4">
            ...
            <div className="mt-2.5 pt-2.5 border-t border-pink-100 flex items-center gap-2 animate-fade-in">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-pink-50/60 border border-pink-200/80 rounded-xl pl-8 pr-7 py-1.5 text-[16px] sm:text-xs text-zinc-800 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-pink-400"
                />
                {searchQuery && (
                  <button
                    type="button"
        ...
        >
                    ...
                    >
                      ...
                      {msg.replyTo && (
                        <div
                          onClick={() => {
                            if (msg.replyTo?.id) {
                              scrollToMessageElement(`msg-${msg.replyTo.id}`);
                            }
                          }}
          ...
          {/* Floating New Incoming Message Pill */}
          {hasUnreadBelow && (
            <div className="sticky bottom-2 left-0 right-0 z-30 flex justify-center pointer-events-none">
              <button
                type="button"
                onClick={() => scrollToBottom(true)}
                className="pointer-events-auto px-4 py-2 rounded-full bg-gradient-to-r from-pink-600 to-rose-500 text-white text-xs font-bold shadow-lg shadow-pink-500/30 flex items-center gap-1.5 animate-bounce cursor-pointer active:scale-95 transition-transform"
              >
                <Sparkles className="w-3.5 h-3.5 text-amber-200" />
                <span>Yeni mesaj geldi ↓</span>
              </button>
            </div>
          )}

          {/* Floating Scroll to Bottom Button */}
          {showScrollBottom && !hasUnreadBelow && (
            <button
              type="button"
              onClick={() => scrollToBottom(true)}
        ...
        <div className="shrink-0 mt-1 mb-0.5">
          ...
          {showQuickPrompts && (
            <div className="flex items-center gap-1.5 overflow-x-auto pt-1 pb-0.5 scrollbar-none animate-fade-in">
        ...
        {/* BOTTOM INPUT BAR - SAFARI OPTIMIZED (Font 16px to prevent iOS auto-zoom jump) */}
        <form
